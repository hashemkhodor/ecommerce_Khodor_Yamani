...
def test_review_model_invalid_flagged_value():
    ...
    assert (
        "Input should be 'flagged', 'approved' or 'needs_approval'" in errors[0]["msg"]
    )
...
def test_put_review_request_invalid_rating():
    with pytest.raises(ValidationError) as exc_info:
        PutReviewRequest(
            customer_id="user123", item_id=456, rating=10  # Invalid rating (>5)
        )
    errors = exc_info.value.errors()
    assert errors[0]["loc"] == ("rating",)
    assert "less than or equal to 5" in errors[0]["msg"]
...
def test_moderate_request_valid():
    moderate_data = {"customer_id": "user123", "item_id": 456, "flag": "approved"}
...
def test_moderate_request_invalid_flag():
    with pytest.raises(ValidationError) as exc_info:
        ModerateRequest(customer_id="user123", item_id=456, flag="invalid_flag")
    errors = exc_info.value.errors()
...
def test_login_response_success():
    ...
    response = LoginResponse(
        status_code=200, credentials_schema=credentials, token=token
    )
...
def test_post_review_response_success():
    review_request = PostReviewRequest(
        customer_id="user123", item_id=456, rating=5, comment="Great product!"
    )
    response = PostReviewResponse(status_code=200, review_schema=review_request)
    assert response.status_code == 200
    content = response.body.decode("utf-8")
    assert (
        f"Posted {review_request.customer_id}'s review for {review_request.item_id} successfully."
        in content
    )
...
def test_post_review_response_conflict():
    ...
    content = response.body.decode("utf-8")
    assert (
        f"{review_request.customer_id}'s review for {review_request.item_id} already exists."
        in content
    )
...
def test_put_review_response_success():
    put_review_request = PutReviewRequest(
        customer_id="user123", item_id=456, rating=4, comment="Updated review."
    )
    response = PutReviewResponse(status_code=200, review_schema=put_review_request)
    assert response.status_code == 200
    content = response.body.decode("utf-8")
    assert (
        f"Updated {put_review_request.customer_id}'s review for {put_review_request.item_id} successfully."
        in content
    )
...
def test_get_reviews_response_success():
    ...
    response = GetReviewsResponse(
        status_code=200, item_id=456, reviews=[review1, review2]
    )
    assert response.status_code == 200
    content = response.body.decode("utf-8")
...
def test_moderate_reviews_response_success():
    ...
    content = response.body.decode("utf-8")
    assert (
        f"Moderate review successfully for item_id {review.item_id} and customer_id {review.customer_id}."
        in content
    )
...
def test_get_reviews_response_invalid_status_code():
    with pytest.raises(ValueError) as exc_info:
        GetReviewsResponse(status_code=404, item_id=456, reviews=[])
