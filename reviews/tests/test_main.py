...
def test_submit_review_success(mock_db):
    ...
    mock_db.submit_review.return_value = [Review(**review_request, flagged="needs_approval")]
...
def test_submit_review_customer_or_item_missing(mock_db):
    ...
    data = response.json()
    assert "Either customer 'customer1' or item with id '1' doesn't exist." in data["message"]
...
def test_update_review_success(mock_db):
    ...
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert f"Updated {updated_review['customer_id']}'s review for {updated_review['item_id']} successfully." in data["message"]
...
def test_update_review_not_found(mock_db):
    ...
    data = response.json()
    assert f"{updated_review['customer_id']}'s review for {updated_review['item_id']} not found" in data["message"]
...
def test_delete_review_success(mock_db):
    mock_db.get_review.return_value = [Review(
        customer_id="customer1",
        item_id=1,
        rating=5,
        comment="Great product!",
    ...
    response = client.delete("/api/v1/reviews/delete", params={"customer_id": "customer1", "item_id": 1})
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
...
def test_get_customer_review_success(mock_db):
    ...
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert f"Fetched reviews successfully for customer_id {customer_id}." in data["message"]
...
def test_get_details_success(mock_db):
    ...
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert "Fetched reviews successfully for item_id 1 and customer_id customer1." in data["message"]
    assert len(data["data"]) == 1
...
def test_moderate_success(mock_db, mock_decode_access_token):
    ...
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert "Moderate review successfully for item_id 1 and customer_id customer1." in data["message"]
...
def test_moderate_unauthorized(mock_db, mock_decode_access_token):
    ...
    mock_decode_access_token.return_value = {"username": "customer1", "role": "customer"}
    ...
    data = response.json()
    assert "User don't have the permission to perform this action" in data.get("errors", "")
